...
public class TorreLanzallamas : TorreBase, IAatacante
{
    public float divisionesRayos = 10;
    ...
    private List<Vector3> ObtenerPuntos()
    {
        ...
        float divisor = 1f / divisionesRayos;
        float linear = 0f;
        bool esPositivo = true;

        if (divisionesRayos == 0)
        {
            Debug.LogError("no podemos dividir entre cero por favor ingresar otro valor en el prefab");
            return null;
        }
        if (divisionesRayos == 1)
        {
        ...
        for (int i = 0; i < divisionesRayos; i++)
        {
            if (i == 0)
            {
                linear = divisor / 2;
            ...
            if (esPositivo)
            {
                punto.x += (Random.value - 0.5f) * 0.2f;
                esPositivo = false;
            }
            else
            {
